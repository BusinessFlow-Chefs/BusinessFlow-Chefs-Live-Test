(function (global) {
  ...
  function canonicalName(name) {
    const s = normal(name);
    for (const [re, out] of aliasRules) {
  ...
  function keyFor(name, unit) {
    return `${canonicalName(name)}|${normal(unit || 'each')}`;
  }
  ...
  function locationFor(category) {
    if (category === 'Protein') return 'Fridge / Freezer';
    if (category === 'Produce') return 'Fridge / Produce';
    if (category === 'Chilled') return 'Fridge';
    if (category === 'Dry Store') return 'Pantry';
    return 'Other';
  }
  ...
  function inventoryItems(state) {
    return Object.values(state.inventory?.items || {}).map(normalizeItem);
  }
    ...
    for (const day of state.menu?.days || []) {
      for (const [meal, dishes] of Object.entries(day.meals || {})) {
        for (const dish of dishes || []) {
          const recipe = recipes.getRecipe(dish.id);
          if (!recipe) continue;
  ...
  function stockForRequirement(req, state) {
    const items = inventoryItems(state).filter(
      (i) => i.ingredientKey === req.key || keyFor(i.name, i.unit) === req.key
    );
    return items.reduce(
      (acc, i) => {
        acc.onHand += i.onHand;
        acc.reserved += i.reserved;
        acc.items.push(i);
        return acc;
      },
  ...
  function plan() {
    ...
    const lines = reqs.map((req) => {
      const stock = stockForRequirement(req, state);
      const available = Math.max(0, round(stock.onHand - stock.reserved));
      const buy = Math.max(0, round(req.required - available));
      return {
        ...req,
        onHand: round(stock.onHand),
        reserved: round(stock.reserved),
        available,
        buy,
        covered: buy <= 0,
        priority: buy > 0 ? 'high' : 'covered'
      };
    });
    const missing = lines.filter((x) => x.buy > 0);
    const covered = lines.filter((x) => x.buy <= 0);
    return {
      generatedAt: nowIso(),
      lines,
      summary: {
        requirements: lines.length,
        toBuy: missing.length,
  ...
  function save(next, action) {
    next.inventory = next.inventory || { items: {} };
    next.provisioning = next.provisioning || { items: [] };
    next.audit = Array.isArray(next.audit) ? next.audit : [];
    next.audit.unshift({
      id: uid('audit'),
      at: nowIso(),
      deviceId: next.sync?.deviceId || 'device_browser',
      action,
      detail: {}
    });
  ...
  function upsertItem(data) {
    ...
    if (!Number.isFinite(item.onHand)) throw new Error('Invalid stock quantity');
    const existingId = data?.id && state.inventory?.items?.[data.id] ? data.id : null;
    const id = existingId || item.id;
    item.id = id;
    item.updatedAt = nowIso();
    state.inventory.items[id] = item;
    save(state, existingId ? 'inventory.item.updated' : 'inventory.item.added');
    return clone(item);
  }

  function removeItem(id) {
    const state = core.getState();
    if (!state.inventory?.items?.[id]) throw new Error('Stock item not found');
    delete state.inventory.items[id];
    save(state, 'inventory.item.removed');
  }

  function adjustItem(id, delta) {
    const state = core.getState();
    const raw = state.inventory?.items?.[id];
    if (!raw) throw new Error('Stock item not found');
    const item = normalizeItem(raw);
    item.onHand = Math.max(0, round(item.onHand + Number(delta || 0)));
    item.updatedAt = nowIso();
    state.inventory.items[id] = item;
    save(state, 'inventory.item.adjusted');
    return clone(item);
  }
  ...
  function setCount(id, count) {
    ...
    if (!raw) throw new Error('Stock item not found');
    const item = normalizeItem(raw);
    item.onHand = Math.max(0, Number(count || 0));
    item.updatedAt = nowIso();
    state.inventory.items[id] = item;
    save(state, 'inventory.item.counted');
    return clone(item);
  }
  ...
  function generateProvisioning() {
    ...
    save(state, 'provisioning.generated');
    return clone(state.provisioning);
  }

  function markPurchased(id, qty) {
    const state = core.getState();
    const line = (state.provisioning?.items || []).find((x) => x.id === id);
    if (!line) throw new Error('Provisioning item not found');

    const amount = Math.max(0, Number(qty ?? line.buy ?? 0));
    let stockItem = inventoryItems(state).find((i) => i.ingredientKey === line.ingredientKey);

    if (!stockItem) {
      stockItem = normalizeItem({
        name: line.name,
        unit: line.unit,
        category: line.category,
        onHand: 0
      });
      stockItem.ingredientKey = line.ingredientKey;
    }

    stockItem.onHand = round(stockItem.onHand + amount);
    stockItem.updatedAt = nowIso();
    state.inventory.items[stockItem.id] = stockItem;
    line.status = 'purchased';
    line.purchasedQty = amount;
    line.purchasedAt = nowIso();
    line.buy = Math.max(0, round(line.buy - amount));
    save(state, 'provisioning.item.purchased');
    return clone(line);
  }

  function getInventory() {
    return inventoryItems(core.getState()).sort(
  ...
  function getProvisioning() {
    ...
    return clone(s.provisioning || { items: [] });
  }

  global.BusinessFlowStock = Object.freeze({
    requirements,
    plan,
    getInventory,
    getProvisioning,
